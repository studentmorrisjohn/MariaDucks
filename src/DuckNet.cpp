...
int DuckNet::setupWebServer(bool createCaptivePortal, String html) {
  ...
  // Custom Routes
  webServer.on("/testConnection", HTTP_GET, [&](AsyncWebServerRequest* request) {
    request->send(200, "text/html", "Server is running and connection is successful!");
  });

  webServer.on("/anotherRoute", HTTP_GET, [&](AsyncWebServerRequest* request) {
    request->send(200, "text/html", "Server is running and connection is successful!");
  });

  webServer.on("/sendLocation", HTTP_POST, [&](AsyncWebServerRequest* request) {
    loginfo("Submitting Form to /formSubmit.json");
   ...
   webServer.on("/submitForm", HTTP_POST, [&](AsyncWebServerRequest* request) {
    loginfo("Submitting Form to /formSubmit.json");
